...
class Riska_uji extends CI_Controller {
	...
	{
		$y['title'] = "Riska Uji K3 Dwi";
		$x['title'] = "Riska Uji K3 Dwi";
		$jum=$this->m_tulisan->get_tulisan_by_kategori(6);;
	    ...
	    $limit=6;
	    $config['base_url'] = base_url() . 'riska_uji/index';
		...
		$x['training'] = $this->m_tulisan->berita_perpage_by_kategori(6,$offset,$limit);
		$this->load->view('user/v_header',$y);
		$this->load->view('user/v_riska_uji',$x);
	...
	function detail($slug){
		...
		$data1 = $this->m_galeri->get_galeri_by_album_id($tulisan_album_id);
		$z=$data1->row_array();
		$album=$z['album_nama'];
		$x['album_nama'] = $album;
		$y['title'] = $judul;
		...
		$this->load->view('user/v_header',$y);
		$this->load->view('user/v_riska_uji_detail',$x);
		$this->load->view('user/v_footer');
	}
}
